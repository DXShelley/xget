...
describe('Fast route', () => {
  ...
  it('preserves POST semantics when a transparent redirect reaches a registered site', async () => {
    ...
    const response = await handleFastRoute(new Request(url, init), url, config);
  ...
  it.each([301, 302, 303, 307, 308])(
    ...
    async status => {
      ...
      const response = await handleFastRoute(new Request(url), url, createConfig());
  ...
  it('resolves relative redirect URLs against the upstream and follows the proxy chain', async () => {
    ...
    expect(location.hash).toBe('#section');
    const destination = await handleFastRoute(new Request(location), location, createConfig());
  ...
  it.each([307, 308])(
    ...
    async status => {
      ...
      const url = new URL('https://fast.dxshelley.fun/?target=https%3A%2F%2Fexample.com%2Fsubmit');
      const init = { method: 'POST', body: 'payload=unchanged' };
      const redirect = await handleFastRoute(new Request(url, init), url, config);
      expect(redirect?.status).toBe(status);
      const next = new URL(redirect?.headers.get('Location') || '', url);
      const response = await handleFastRoute(new Request(next, init), next, config);
